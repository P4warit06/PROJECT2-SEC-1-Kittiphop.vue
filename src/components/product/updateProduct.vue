...
const emit = defineEmits(["saveUpdate", "cancel"]);
...
// เริ่มต้นให้ editedProduct มีค่า placeholder
const editedProduct = ref({
    id: route.params.productId,  // ตัวอย่างตั้งต้น
    name: route.params.name,  // ถ้าไม่มีค่าให้ใช้ชื่อเริ่มต้น
    category: route.params.category,  // ใช้ค่า placeholder
    price: route.params.price,  // ใช้ค่าเริ่มต้นเป็น 0
    status: route.params.status,  // ใช้ค่า placeholder
    stock: route.params.stock,  // ใช้ค่าเริ่มต้นเป็น 0
    image: route.params.image,
    description: route.params.description,
});

watch(
    () => route.params,  // ใช้ route.params แทน route.query สำหรับการส่งข้อมูลผ่าน params
    (newProduct) => {
...
const saveChanges = () => {
    // ส่งข้อมูลที่ถูกแก้ไขกลับไปยัง parent component
    emit('saveUpdate', editedProduct.value);
};
</script>
            ...
            <div class="grid grid-cols-1 overflow-auto px-1 sm:grid-cols-2 gap-8 w-full">
                ...
                <div class="col-span-1 sm:col-span-2 flex gap-8 justify-center mt-4">
                    <button @click="saveChanges"
                        class="w-full sm:w-2/3 bg-indigo-500 text-white py-3 rounded-xl hover:bg-indigo-600 cursor-pointer">Save
                        Changes</button>
